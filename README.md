...
- Strings: allocated with `alloc_array8`, item 0 holds length, item 1 first character.
  `set_string(STR, "string")`, `clear_string(STR)`, `append_string(DST, SRC)`, `left_string(DST, SRC, size)`, `right_string(DST, SRC, size)`, `mid_string(DST, SRC, start, size)`, `cmp_string(STR1, STR2, result)`, `print_string(STR)`, `read_word_string(STR)`.
