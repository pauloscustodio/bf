...
# mul16 - error no arguments
spew("$test.in", "mul16");
capture_nok("bfpp $test.in", <<END);
$test.in:1:6: error: expected '(' after macro name 'mul16'
END

# mul16 - error empty arguments
spew("$test.in", "mul16()");
capture_nok("bfpp $test.in", <<END);
$test.in:1:8: error: macro 'mul16' expects 2 arguments
END

# mul16 - error too many arguments
spew("$test.in", "mul16(A,B,C)");
capture_nok("bfpp $test.in", <<END);
...
[ Generated by bfpp, see https://github.com/pauloscustodio/bf
  96800 instructions, 53 tape cells
]
[-]>[-]>[-]>[-]>[-]>[-]>[-]>[-]>[-]>[-]>[-]>[-]>[-]>[-]<+>>[-]>[-]<++<<<<<<<<<<[
-]>[-]>>>>>>>>>>>[-]<<<<<<<<[-]<<<<<<<<[->>>>>>>>+>>>>>>>>+<<<<<<<<<<<<<<<<]>>>>
>>>>>>>>>>>>[-<<<<<<<<<<<<<<<<+>>>>>>>>>>>>>>>>][-]<<<<<<<[-]<<<<<<<<[->>>>>>>>+
